...
exports.PATTERN_EMAIL = /\b[a-z0-9\!\#\$\%\&\'\*\+\/\=\?\^\_\`\{\|\}\~\-]+(?:\.[a-z0-9\!\#\$\%\&\'\*\+\/\=\?\^\_\`\{\|\}\~\-]+)*@(?:[a-z0-9](?:[a-z0-9\-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9\-]*[a-z0-9])?\b/i;
...
function parse(str, pattern, title) {
    ...
    while (match = pattern.exec(str)) {
        if (match.index > 0) {
            result.push(new Token(str.substr(0, match.index)));
        }
        result.push(new Token(title || match[0], match[0]));
